...
\title{A quantum algorithm for solving computational fluid dynamics problems}% Force line breaks with \\
...
\begin{abstract}
    Computational fluid dynamics (CFD) is a branch of fluid mechanics that uses numerical computation to analyze fluid flows. When the number of mesh number grows, the requirement of the computing resources becomes more expensive. We found that if the classical solver was partially replaced by the quantum approach, speedup would be achieved. Here, we propose a quantum algorithm to solve CFD problems, which exponentially faster than the classical solver. We analyzed the time complexity and requirement of qubit resources of our method and simulated a specific CFD problem to prove the result. Our algorithm accelerates the development of CFD and provides a reference for the application of quantum computing in other fields.
% \begin{description}
% \item[Usage]
...
Here we introduce our algorithm. The modules that we replace are the data structures, preconditioners and the linear solvers. In the data structure, these are replaced by quantum memory or quantum states: the information about the mesh grid as the input, all vectors that are involved in the linear equations. In the iteration of the time integration, the preconditioners and the linear solvers of the equations are replaced by a quantum preconditioner and a linear solver. We do not change the problem model, the mesh settings, the time integration and the iteration. As the result, the time complexity of our algorithm is proportional to log⁡N, where N denotes the mesh number. The qubit number is also proportional to log⁡N. Besides the effect of the mesh number, both the complexity and the qubit number is affected by the precision ϵ. The conditional number and the sparsity are considered as a constant because it will not change greatly in this problem.

Besides the algorithm detail, we will discuss the difficulties of the problem. One is that we prove that it is necessary to include the quantum random access memory (QRAM), without which quantum speed up will not be achieved. Another is that the qubit should be fault tolerant logical qubit, because the requirements of the gate number and the state storage time greatly exceed any limit of the near-future NISQ device.



\section{Submission Summary (layman terms)[2-3 pages]}

\subsection{Overall algorithm summary}

\subsubsection{Replacement of the SU2} 

submodule SU2 is an open-source fluid dynamics solver. We study its $SU2\_CFD$ module to solve the computational fluid dynamic problem and the target is to replace some modules in the classical solver by quantum subprocess to form a quantum-classical hybrid software, finally to demonstrate the quantum speed-up. Except for the assistive modules to parse the input file or display the results, the $SU2\_CFD$ solver mainly includes 1 class of solver and 3 classes of data structures:

Process: class CSolver
Data Structure: class CConfig, CGeometry and CNumerics
\begin{figure*}[htbp]
...
The preconditioner and linear solver in the CSolver class are replaced by the quantum process. As the result, the corresponding data structures should be replaced by the quantum data (quantum memory). It includes the mesh settings and the interval vectors. The iteration loop in the CSolver class is not replaced by any quantum process.

% Our algorithm is a quantum version of  finite volume method in CFD problem. We use quantum circuit to realize linear equations construction and solution process. In this section, we show the whole process of our algorithm. The flow chart is shown in fig.~\ref{CfdFlowChart}
\begin{figure}[htbp]
    \subfigure{\includegraphics[width=0.48\textwidth]{Fig/quantum_cfd_flow_chart.png}}
