...
There were several QRAM models proposed in the previous works. A typical one is to implement the QRAM by a set of optical devices, requiring $\mathcal{O}(log N)$ operations (N denotes the memory length). Another more straightforward approach is to realize it with the decomposition of the quantum circuits. However, although the addressing part is realized in $\mathcal{O}(log N)$, the state generation part involves $\mathcal{O}(N)$ multi-controlled NOT gates, which should not be considered as an efficient plan. Here we will introduce an efficiently QRAM only requiring $\mathcal{O}(log N)$ operations in our plan.
...
For the fourth case of the same table, we claim such a plan impossible to implement in practice. The problem is that the complexity of the query of $O_A$ should be constructed from the previous solved vector $\vec{u}^{n-1}$. As mentioned above, $O_A$ is related to more than one component in $\vec{u}^{n-1}$ for each matrix element, and we shall prove formally that this process cannot be implemented by a linear operator. Such operators are considered fundamental in quantum mechanics. The proof is developed in Supplementary Information.
...
\subsection{Overall Circuit and Qubit Number}
...
In Fig.~\ref{o}, $t=\mathcal{O}(log(j_0))$ is the qubits for control bits $\frac{1}
{\sqrt{\alpha}}\sum_i\sqrt{\alpha_i}|i\rangle$. The total qubits number is 
\begin{equation}
2(m+1)+t+C_Aq_f
\end{equation}

\subsection{Time Complexity and Successful Probability}
...
$\mathcal{O}(j_0)=\mathcal{O}(d\kappa log(d\kappa/\epsilon))$, the total
query complexity is $\mathcal{O}(\alpha j_0)$. Because $\alpha\leqslant 4j_0/d$,
$\mathcal{O}(\alpha j_0)=\mathcal{O}(d\kappa^2log^2(d\kappa/\epsilon))$. The total number of 
$\mathcal{P}_B$ is $\mathcal{O}(\alpha)$.

In \cite{7354428}, the gate complexity is $\mathcal{O}(d\kappa^2log^2(d\kappa/
...
so $\lVert F|b\rangle\rVert\in (1, \kappa)$.
\begin{equation}
...
\subsection{Proof of Theorem 1 and 2}
