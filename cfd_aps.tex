...
\begin{equation}
O(\text{qnum})=O(2\log(4N)+\log(d\kappa \log(d\kappa/\epsilon)))
\end{equation}
The QRAM and oracle query complexity is 
\begin{equation}
O(\text{query})=O(d\kappa^2\log^2(d\kappa/\epsilon))
\end{equation}
The gate complexity is 
\begin{equation}
O(d\kappa^2\log^2(d\kappa/\epsilon)\times poly(\log(4N)/\epsilon))
\end{equation}
...
\item $\vec{u}_i$: variables on the mesh $i$ 
\end{itemize}
...
For briefness, we assumed the flow to be 2D, inviscid and steady, then gained the Euler equation \cite{euler1757principes}, a special form of the Navier-Stokes equations:
