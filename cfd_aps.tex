...
\begin{equation}
\mathcal{O}(d\kappa^2\log^2(d\kappa/\epsilon)\times \text{poly}(\log(4N)/\epsilon))
\end{equation}
...
\item $\vec{U}=\{\vec{U}_i, i=0,1,2,...,N-1\}$, $\vec{U}$ represents all variables of the problem, the dimension is $4N$.
\end{itemize}
...
The meshing scheme oracle is used to encode the meshing scheme into the quantum circuit. We define two such oracles: $O_{\rm adjacent}$ and $O_{\rm coor}$. $O_{\rm adjacent}$ is used to extract the adjacent grid number of a specified grid and is written
\begin{equation}
O_{\rm adjacent}|i\rangle|j\rangle=|i\rangle|g(i,j)\rangle,
\end{equation}
that is, the $j^{th}$ adjacent grid number of grid $i$ is $g(i,j)$. $O_{\rm coor}$ is used to extract the coordinates of a grid,
\begin{equation}
O_{\rm coor}|i\rangle|0\rangle=|i\rangle|\vec{x}_i\rangle.
\end{equation}
Here $\vec{x}_i$ denotes the coordinates of each corner of grid $i$. We can get other positional information with $O_{\rm coor}$, such as the coordinates for the center of gravity of the grid and the normal to each edge of the grid $i$.

The number of qubits required to construct $O_{\rm adjacent}$ and $O_{\rm coor}$ is $2(m-2)$, $m-2+C_0q_f$, respectively; here $C_0q_f$ is a qubit sequence representing coordinate information and other positional information obtained from the grid coordinates, and $C_0$ is a constant number.

The construction complexity of $O_{\rm adjacent}$ and $O_{\rm coor}$ is related to the complexity of the meshing scheme. In our method, we assume this complexity is $\mathcal{O}({\rm poly}(\log(N)))$, so the complexity of $O_{\rm adjacent}$ and $O_{\rm coor}$ is $\mathcal{O}({\rm poly}(\log(N)))$. 
...
In the preprocessing step, we construct the oracles that represent matrix $\bm{A}$ and vector $\vec{b}$ in one iteration process. In this step, we use the QRAM $M_U$ and oracles $O_{\rm adjacent}$ and $O_{\rm coor}$ to construct $O_l$, $O_b$, and $O_A$.
...
\end{equation}
where $f(i,j)$ represents the column number of the $j^{th}$ nonzero element of the $i^{th}$ row of $\bm{A}$. We use $O_{\rm adjacent}$ to construct $f(i,j)$, which may be written as
\begin{equation}
f(i,j)=4g(i/4,j/4)+j\%4.
\end{equation}
There is a trick in constructing the quantum circuit of this process (Fig.~\ref{fig:OL}). We simply operate $O_{\rm adjacent}$ on the highest $\log(N)$ qubits of $i$ and $j$; then, in the entire space of $|i\rangle$ and $|j\rangle$, we have constructed $O_l$. The required number of qubits is $2m$ in this process.
...
\end{equation}
As stated in Sec.~\ref{classicalFVM}, $\vec{b}$ represents the residual $\vec{R}$. We find $R_i$ is a function of variables and coordinates of the $(i/4)^{th}$ cell and its adjacent cells. In particular, the expression for these functions is regular; specifically, the expression for $b_i$ is only related to $i\%4$ , so we can construct $b_i$ efficiently. In detail, we use $M_U$, $O_l$, $O_{\rm coor}$ to construct $O_b$ (Fig.\ref{fig:OB}). First, we get the related variables and coordinates,
\begin{equation}
|i\rangle|U_{\rm related}\rangle|x_{\rm related}\rangle.
\end{equation}
Then, we compute $b_i$ and obtain
\begin{equation}
|i\rangle|U_{\rm related}\rangle|x_{\rm related}\rangle|b_i\rangle.
\end{equation}
Next, we uncompute $|U_{\rm related}\rangle$ and $|x_{\rm related}\rangle$,
\begin{equation}
|i\rangle|b_i\rangle.
\end{equation}
...
In this process, the required number of qubits is $m+C_bq_f$, $C_bq_f$ is an ancilla qubit sequence used to compute $b_i$; it can also be used in QDAC quantum circuit. We assume the success rate of QDAC is $p_{QDAC}$, then the number of times $M_U$, $O_{\rm adjacent}$ and $O_{\rm coor}$ are used is $\mathcal{O}(1/p_{QDAC})$. The gate complexity is $\mathcal{O}({\rm poly}(m)/p_{QDAC}+{\rm poly}(1/\epsilon p_{QDAC}))$, with $\epsilon$ representing the QDAC accuracy.
...
\end{equation}
To construct $O_A$, we also need $M_U$, $O_l$, and $O_{\rm coor}$. As stated in Sec.~\ref{classicalFVM}, matrix $\bm{A}$ is sparse and the element $A_{4i_0+i_1,4j_0+j_1} $ is nonzero only if grid $i_0$ and $j_0$ are adjacent; here $i_0,j_0\in[0,N-1]$ and $i_1,j_1\in[0,3]$, and hence we have $i=4i_0+i_1$ and $j=4j_0+j_1$. Element $A_{4i_0+i_1,4j_0+j_1}$ is a function of variables and coordinates of the grid $i_0$ and $j_0$ that may be written
\begin{equation}
...
The overall quantum circuit of our quantum linear solver is shown in Fig.~\ref{fig:o}. The details of this quantum circuit is explained in Sec.\ref{MatrixInverse}.
...
\begin{equation}
p_{QDAC}=\frac{1}{4N}\sum_i{(\Delta U_i/C_{\rm max})^2}
\end{equation}
and
\begin{equation}
p_{A^{-1}}=\frac{1}{\alpha}.
\end{equation}
Here $C_{\rm max}>=\max\{\Delta U_i\}$ and $\mathcal{O}(\alpha)=\mathcal{O}(\kappa\log(\kappa d/\epsilon))$. $p_{QDAC}$ is a constant number and hence can be ignored. Oracles $O_{\rm adjacent}$ and $O_{\rm coor}$ appear $\mathcal{O}(1)$ times in the quantum circuit of $O_l,O_b,O_A$, and $O_l$,$O_A$ appears $\mathcal{O}(j_0)$ times in each iteration. Hence, the query complexity for both $O_{\rm adjacent}$ and $O_{\rm coor}$ is $\mathcal{O}(\alpha\times {\rm ITimes}\times j_0)$.

QRAM $M_U$ also appears $\mathcal{O}(1)$ times in constructing $O_b$ and $O_A$, the query complexity of $M_U$ being $\mathcal{O}(\alpha {\rm ITimes}\times j_0)$. In each iteration, we update $M_U$, the update complexity of $M_U$ being $\mathcal{O}({\rm ITimes})$.

The gate complexity of the whole process is $\mathcal{O}({\rm ITimes}\times\alpha j_0poly(\log(4N))/\epsilon)$.
...
\begin{figure}[htbp]
 ...
 \subfigure[Quantum gate $Q$]{\includegraphics[width=0.35\textwidth]{Fig/Q.png}}
...
$\frac{1}{{\sqrt N }}\sum\limits_{k = 0}^N {{{\left| k \right\rangle }_{\text{ad}}}{{\left| {{{\tilde {\textbf x}}_k}} \right\rangle }_{\text{reg}1}}{{\left| 0 \right\rangle }_{\text{reg}0,\text{data},B}}} $
is obtained.
...
\subsubsection{Implementing the matrix inverse by linear combination of operators}\label{MatrixInverse}
...
\end{equation}
where $j_0=\sqrt{b \log(4b/\epsilon)}$ and $b=\kappa^2\log(\kappa/\epsilon)$.
...
\begin{equation}
|l\rangle|z\rangle\xrightarrow{O_{\text{amp}}}|l\rangle|z\otimes\alpha_l^{(n)}
\rangle\notag
\end{equation}
Our goal is to prepare an approximation to the target state
\begin{equation}
|\text{target}\rangle:=\frac{1}{{\lVert\vec{\alpha}\rVert}_2}\sum_{l=0}^n\alpha_l
|l\rangle\notag
\end{equation}
...
\end{align}
\begin{align}
K&=2|0\rangle\langle0|-I_{2N}\notag \\
&=diag(1,-1,-1,\dots,-1)=-diag(-1,1,1,\dots,1)\notag
\end{align}
\begin{figure}[htbp]
	\subfigure[Reflection operator $K$]{\includegraphics[width=0.20\textwidth]{Fig/K.png}}
	\subfigure[$W$ operator]{\includegraphics[width=0.45\textwidth]{Fig/WW.png}}
	\caption{Quantum circuit of operator $K$ and $W$}
	\label{W}
\end{figure}
